...
<section class="login">
  <div class="from">
    ...
    <p class="input">
      ...
      <input type="text" id="userName" name="userName" placeholder="用户名" v-model.trim ="name">
    </p>
    <p class="input">
      <i class="fa fa-key fa-fw"></i>
      <input type="password" id="password" placeholder="密码" v-model.trim="pwd">
    </p>
     <p class="info">{{info}}</p>
     <p>
...
export default {
  ...
  methods:{
    doLogin(){
      if (!this.name.length) {
        return this.info = "请输入正确的用户名"
      }
      if (!this.pwd.length) {
        return this.info ="请输入正确的密码"
      }
